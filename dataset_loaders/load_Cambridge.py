...
def fix_coord(args, train_set, val_set, pose_avg_stats_file='', rescale_coord=True):
    ''' fix coord for Cambridge Landmarks to align with llff style dataset '''
...
def load_Cambridge_dataloader(args):
    ...
    kwargs = dict(args=args, scene=scene, data_path=data_dir,
        ...
        ret_hist=ret_hist, hist_bin=args.hist_bin, ret_semantic=args.semantic, color_jitter=color_jitter)
    ...
    ### This is important if we want faster data loading ###
    train_set.preload_data()
    val_set.preload_data()

    train_shuffle=True
    if args.eval:
        train_shuffle=False

    train_dl = DataLoader(train_set, batch_size=args.batch_size, shuffle=train_shuffle, num_workers=8)
...
def load_Cambridge_dataloader_NeRF(args):
    ...
    if 'color_jitter' in args:
        assert(args.color_jitter==False) # currently not support color jitter for nerf training
        if args.color_jitter:
            color_jitter = True
    ...
    kwargs = dict(args=args, scene=scene, data_path=data_dir,
        transform=data_transform, target_transform=target_transform, 
        df=args.df, ret_idx=ret_idx, fix_idx=fix_idx, ret_hist=ret_hist, hist_bin=args.hist_bin, ret_semantic=args.semantic, color_jitter=color_jitter)
    ...
    ### Attempt for faster data loading ###
    train_set.preload_data()
    val_set.preload_data()
    ...
    if args.render_video_train or args.render_test or args.dataset_type == 'Cambridge2':
        train_shuffle=False
    train_dl = DataLoader(train_set, batch_size=args.batch_size, shuffle=train_shuffle, num_workers=4) # default
