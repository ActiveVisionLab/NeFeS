# courtsey to dsac* https://github.com/vislearn/dsacstar/
import os
import math
...
semantic_file = semantic[0].split('/')[-1]
...
for scene in scenes:
	...
	os.chdir('..')
	os.system(f'mv Cambridge_{scene_name}/ Cambridge/{scene_name}')
	os.system(f'cp Cambridge_world_setup/{scene_name}/* Cambridge/{scene_name}/')

# put semantic/ folders to Cambridge scenes
os.system('wget ' + semantic[0])
os.system('unzip ' + semantic_file)
os.system('rm ' + semantic_file)
for scene in scenes:
	scene_file = scene.split('/')[-1]
	scene_name = scene_file[:-4]
	os.system(f'cp -d -r Cambridge_semantic/{scene_name}_semantic_train Cambridge/{scene_name}/train/semantic')
	os.system(f'cp -d -r Cambridge_semantic/{scene_name}_semantic_test Cambridge/{scene_name}/test/semantic')
